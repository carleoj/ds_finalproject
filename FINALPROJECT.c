...
typedef struct {
 ...
 void enqueue(Queue *queue);                //Queue operation to insert a record at the tail
 ...
 void searchRecordByStudentID(sqlite3 *database,
      const char *studentID);                       //print the database with the student ID as the argument
...
int main() {
    ...
    while (choice != 8) {
        ...
        switch (choice) {
            case 1:
                enqueue(queue);             //lets user input record fields
                break;
            case 2:
...
void enqueue(Queue *queue) {
    ...
    // Check if the student ID already exists in the queue
    for (int i = queue->front; i <= queue->rear; i++) {
        if (strcmp(queue->data[i].id, newID) == 0) {
            printf("Student ID already exists in the queue:\n\n");
            printf("Name: %s", queue->data[i].name);
            printf("Course: %s\n", queue->data[i].course);

            strncpy(queue->data[queue->size].name, queue->data[i].name, sizeof(queue->data[queue->size].name) - 1);
            strncpy(queue->data[queue->size].course, queue->data[i].course, sizeof(queue->data[queue->size].course) - 1);

            // Input only the book name
            printf("Enter the new book: ");
            fgets(queue->data[queue->size].book, 30, stdin);

            printf("\nNew Record Enqueued!\n");
            // Remove the newline character from the book name
            queue->data[queue->size].book[strcspn(queue->data[queue->size].book, "\n")] = '\0';

            // Use the previous date
            memcpy(&(queue->data[queue->size].datetime), &(queue->data[i].datetime), sizeof(struct tm));

            strncpy(queue->data[queue->size].id, newID, sizeof(queue->data[queue->size].id) - 1);

            // Increase size and rear
            queue->rear++;
            queue->size++;
    ...
    // If the student ID is not found, input all fields
...
void processDatabase(sqlite3 *database) {
    ...
    switch (dbOp) {
        case 1:
            ...
            fgets(input, sizeof(input), stdin);
            trimNewline(input);
            searchRecordByStudentID(database, input);
            break;
        case 2:
...
void searchRecordByStudentID(sqlite3 *database, const char *studentID) {
...
void delay(int seconds) { //function for visual aids enhancements in the program
    int i;
    for (i = 0; i < seconds * 3; i++) { // Multiply seconds by 10 for 0.3 second intervals
        printf(".");
